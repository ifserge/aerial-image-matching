...
with ThreadPool(processes=cv2.getNumberOfCPUs()) as pool:
    for fl in data_df['id'].values[:300]:
...
for i_img,(fl, x, y, a) in enumerate(preds):
    ...
    if len(kp1) < 1000:
        print(f'image {i_img} has just {len(kp1)} keypoints')
        continue
    ...
    for train_img, (kp2, desc2) in train_dict.items():
        candidates.append((train_img, kp1, desc1, kp2, desc2))
            
    with WorkerPool(n_jobs=20, shared_objects=matcher) as pool:
